...
def item_query(doctype, txt, searchfield, start, page_len, filters, as_dict=False):
	...
	return frappe.db.sql(
		...
		{columns},IFNULL((select sum(sle.actual_qty) from `tabBin` sle where sle.item_code = tabItem.name and sle.warehouse in (select W.name from `tabWarehouse` W where W.is_reserve_warehouse = 0) group by sle.item_code),0) as qty
		from tabItem

...
def get_same_category(product_id):
    ...
    data = frappe.db.sql("""
        ...
        i.item_name,
        i.item_group,
        COALESCE((SELECT SUM(sle.actual_qty) FROM `tabBin` sle WHERE sle.item_code = i.name AND sle.warehouse IN (SELECT W.name FROM `tabWarehouse` W WHERE W.is_reserve_warehouse = 0) GROUP BY sle.item_code), 0) AS qty
        FROM
        `tabItem` i
        WHERE
        i.stock_category = '%s'
...
def get_godown_wise(product_id):
    ...
    if sn_bn['has_batch_no']:
        actual_data = frappe.db.sql("""
        SELECT bn.warehouse, bn.actual_qty, bn.item_code, wh.is_reserve_warehouse, wh.warehouse AS og_warehouse 
        FROM `tabBin` bn 
        INNER JOIN `tabWarehouse` wh ON wh.name = bn.warehouse 
        WHERE bn.actual_qty > 0 AND bn.item_code = %s
        """, product_id, as_dict=1)
        raw_batch_data = []
        warehouse_data = []
        for item in actual_data:
            ...
            for b in batch:
                ...
                b['is_reserve_warehouse'] = item.is_reserve_warehouse
                b['og_warehouse'] = item.og_warehouse
                if b['qty'] > 0:  # Check if quantity is greater than 0
                    raw_batch_data.append(b)
        for item in raw_batch_data:
            if item.is_reserve_warehouse:
                if any(d['warehouse'] == item.og_warehouse and d['batch_id'] == item.batch_id for d in warehouse_data):
        ...
        actual_data = frappe.db.sql("""
        SELECT bn.warehouse, bn.actual_qty, wh.is_reserve_warehouse, wh.warehouse AS og_warehouse 
        FROM `tabBin` bn 
        INNER JOIN `tabWarehouse` wh ON wh.name = bn.warehouse 
        WHERE bn.actual_qty > 0 AND bn.item_code = %s 
        """, product_id, as_dict=1)
